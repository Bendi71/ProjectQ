...
for i in range(5):
    ...
    adatok.extend([final[1],values.mean(),np.sqrt(values.std()),values.min()])
...
with pd.option_context('display.max_columns',None):
    print(pd.DataFrame(eredmenyek,columns=['Time','Return','Mean Return','Deviation','Max Drawdown','VaR 5%','cVaR 5%','VaR 10%','cVaR 10%']))